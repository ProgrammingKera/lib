...
function checkLoginAttempts($conn, $identifier, $ipAddress) {
    ...
    $cleanupSql = "DELETE FROM login_attempts WHERE attempt_time < DATE_SUB(NOW(), INTERVAL 30 SECOND)";
    ...
    if ($row['failed_attempts'] >= 5) {
        ...
        if ($timeDiff < $blockDuration) {
            return array(
                'blocked' => true,
                'remaining_time' => $blockDuration - $timeDiff,
                'message' => 'Failed attempts. Please wait ' . ($blockDuration - $timeDiff) . ' seconds before trying again.'
            );
        }
    }
